...
def naver_search(rb):   # 네이버에 지식백과를 이용한 의약품 용도 검색
    if app.getEntry("searching_option_item")=='':
        app.errorBox("경고", "검색창에 약물을 입력해주세요.")
    else:
        if app.getRadioButton("drug")=='약품 이름검색':
            ...
            if ImageUrl==None:
                ...
                naverurl = 'https://search.naver.com/search.naver?where=kdic&sm=tab_jum&ie=utf8&query=' + CLASS_NAME
                new = 2
                webbrowser.open(naverurl,new=new)
        else:
            app.errorBox("경고", "약품 이름 검색으로만 가능한 서비스입니다.")
...
app.addRadioButton("위키한영전환", "한글위키",14,1)
app.addRadioButton("위키한영전환", "영문위키",14,2)
app.addButton("네이버 용도 검색",naver_search,15,)
app.addButton("이메일전송",sendemailbutton,16,)
app.go()
